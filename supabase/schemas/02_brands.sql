...
comment on column public.brands.slug is 'URL-friendly slug autogenerated from brand name, guaranteed unique';
...
-- Function to generate a unique slug from brand name
create or replace function public.generate_brand_slug(brand_name text, brand_id bigint default null)
returns text
language plpgsql
as $$
declare
...
comment on function public.generate_brand_slug is 'Generates a unique URL-friendly slug from brand name, handling conflicts with numeric suffixes';

-- Function to automatically set slug before insert/update
...
comment on function public.set_brand_slug is 'Trigger function to automatically generate brand slug from name on insert/update';

-- Create trigger to auto-generate slug
create trigger trigger_brands_set_slug
